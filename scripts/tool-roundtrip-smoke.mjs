...
let call
for await (const chunk of adapter.stream({ provider: 'claude-subscription', model: 'sonnet', messages: [first], tools: [tool] })) {
...
let text = ''
let finish
for await (const chunk of adapter.stream({ provider: 'claude-subscription', model: 'sonnet', messages: history, tools: [tool] })) {
