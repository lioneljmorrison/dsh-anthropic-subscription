...
export function classifyClaudeFailure(message) {
    const text = message.toLowerCase();
    if (/log[ -]?in|login required|not authenticated|authentication|unauthorized|expired/.test(text))
        return 'AUTHENTICATION';
    if (/rate limit|rate_limit|too many requests|five-hour|weekly limit|usage limit|try again later/.test(text))
        return 'RATE_LIMIT';
    if (/unknown model|model .* not found|invalid model/.test(text))
...
function systemFingerprint(options) {
    return createHash('sha256').update(systemText(options) ?? '').digest('hex');
}
export function continuationRequest(options) {
    if (options.sessionId === undefined)
        return { options, sessionId: undefined, resume: false };
...
export async function* runClaude(options, config) {
    ...
    if (continuation.sessionId === undefined)
        args.push('--no-session-persistence');
    else if (continuation.resume)
        args.push('--resume', continuation.sessionId);
    else
        args.push('--session-id', continuation.sessionId);
